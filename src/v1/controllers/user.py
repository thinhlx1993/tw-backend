...
user_ns2 = api_version_1_web.namespace(
    'user',
...
token_expired_response_model = user_ns2.model(
    "token_expired_response_model", {
        "message": fields.String(example="Token expired")
    }
)
...
user_id_parser.add_argument('user_id', type=str,
                            help='User id of the user', location='args')
dashboard_theme_parser = reqparse.RequestParser()
dashboard_theme_parser.add_argument('theme', type=str, default='dark',
                                    help='Theme of the dashboard', location='args')
user_model = {
...
register_user_model = {
    ...
    "first_name": fields.String(example="John"),
    "last_name": fields.String(example="Doe"),
    "role_id": fields.String(example="b40ee1ae-5a12-487a-98cc-b6d07238e17a", required=True),
...
user_reset_password_model = user_ns2.model("user_reset_password_model", {
...
google_login_parser.add_argument('login', type=str,
                                 help='login', location='args')

callback_parser = reqparse.RequestParser()
callback_parser.add_argument('code', type=str,
                             help='code', location='args')
callback_parser.add_argument('scope', type=str,
                             help='scope', location='args')

# User login model
user_password_model = user_ns2.model("login_model", {
    "username": fields.String(example="thinhle.ict", required=True),
    "password": fields.String(example="Admin@1234", required=True)
})
user_register_model = user_ns2.model(
    'user_register_model', register_user_model)
...
user_update_model.update({
    "password": fields.String(example="new password")
})

user_notification_model = user_ns2.model("user_notification_model", {
    "user_id": fields.String(example=str(uuid.uuid4()), required=True),
    "notification_status": fields.Boolean(
        example=True, required=True)
})

forgot_password_model = user_ns2.model("forgot_password_model", {
...
validate_user_email_verification_model = user_ns2.model("validate_user_email_verification_model", {
    "token": fields.String(example="eyJhbGciOiJIUA", required=True)
})
...
reset_password_model = user_ns2.model("reset_password_model", {
    "token": fields.String(example="eyJhbGciOiJIUA", required=True),
...
user_invite_ok_model = user_ns2.model(
    'user_invite_ok_model', {
        'message': fields.String(example='Invite mail sent!')
    }
)
...
mfa_activate_model = user_ns2.model("mfa_activate_model", {
    "otp": fields.String(example='248762')
})

mfa_verify_model = user_ns2.model("mfa_verify_model", {
    "otp": fields.String(example='426871')
})
...
user_google_auth_redirect_model = user_ns2.model(
    'user_google_login_redirect_model', {
        'message': fields.String(example='Redirect')
    }
)
...
user_google_login_response_ok_model = user_ns2.model(
    'user_google_login_response_ok_model', {
        ...
        'refresh_token': fields.String(example='eyJ0eXAiOiJKV1QiLCJhbGciOi'
...
user_google_login_bad_response_model = user_ns2.model(
    'user_logout_response_ok_model', {
        'message': fields.String(example='An unexpected \
...
users_list = {
    "user_list": fields.List(fields.Nested(user_ns2.model(
        'users_list_obj',
        users_list_obj)))
}
...
orgs_list = {
    "org_list": fields.List(fields.Nested(user_ns2.model(
        'user_org_dict',
        user_org_dict)))
}
...
user_dashboard_response_ok_model = user_ns2.model(
    'user_dashboard_response_ok_model', {
        'dashboard_url': fields.String(example=
                                       'https://domain/d/Sh4lSPFVk/test')
    }
)
user_dashboard_bad_response_model = user_ns2.model(
    'user_dashboard_bad_response_model', {
        'message': fields.String(example='user_id')
    }
)
        ...
        try:
            ...
            if not user_details:
                return {
                    ...
                    .format(input_username)}, 200
            if not user_services.validate_password(
                    input_username, input_password):
                return {
                    "message": "Username and password combination not valid"
                }, 200
            user_services.update_user_last_active_at(
    ...
    def post(self):
        try:
            ...
            if (user_details.get('first_name') is not None
                ...
                profile_name = str(user_details.get(
                    'first_name', '') + ' '
...
class UserGet(Resource):
    ...
    @user_ns2.response(
        ...
        unauthorized_response_model)
    @user_ns2.response(
        401, "Token expired", token_expired_response_model)
    @user_ns2.response(
        500, "Internal Server Error", internal_server_error_model)
...
class UserSwitchTeams(Resource):
    ...
    @user_ns2.response(
        400, "Bad Request", user_switch_org_bad_response_model)
    @user_ns2.response(
        401, "Authorization information is missing or invalid.",
        unauthorized_response_model)
    @user_ns2.response(
        ...
        try:
            ...
            if super_admin == user_id:
                ...
                token = {'access_token': create_access_token(
                    {**user_payload, 'refresh_jti': get_jti(refresh_token),
                     'type': 'access'}),
                    'refresh_token': refresh_token}
                return token, 200
            elif user_services.check_user_teams_mapping(user_id,
                                                        teams_id):
