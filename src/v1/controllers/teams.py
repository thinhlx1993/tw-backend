...
org_ns2 = api_version_1_web.namespace("teams", description="Teams Functionalities")
...
org_fetch_response_ok_model = org_ns2.model(
    ...
    {
        "data": fields.List(
            fields.Nested(org_ns2.model("org_fetch_dict", org_fetch_dict))
        )
    },
)
...
org_create_model = org_ns2.model(
    "org_create_model", {"teams_name": fields.String(example="Cognicept Systems")}
)
...
org_delete_row_model = org_ns2.model(
    "org_delete_row_model", {"message": fields.String(example="Deleted successfully")}
)
...
teams_post_ok_model = org_ns2.model(
    "teams_post_ok_model",
    {"message": fields.String(example="User user@gmail.com has been added to Kabam")},
)
...
class Teams(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            sort_by = str(args.get("sort_by")) if args.get("sort_by") else "teams_name"
            ...
            filters = json.loads(args.get("filter")) if args.get("filter") else None
            ...
            if user_id == UserTypeEnums.SuperAdmin.value:
                ...
                status, data = teams_services.search_user_org_list(
                    user_id,
                    page=page,
                    per_page=per_page,
                    sort_by=sort_by,
    ...
    def post(self):
        ...
        status, teams_id, err_msg = teams_services.create_teams(teams_name, user_id)

        try:
            # Create teams and schema for user
            if status:
                # Create user teams mapping with is_default=False
                if user_services.create_user_teams_mapping(user_id, teams_id, False):
                    ...
                    return {"message": "Error creating user_org_mapping"}, 500
            elif err_msg:
        ...
        try:
            ...
            user_services.create_user_teams_mapping(user_id, teams_id, False)
    ...
    def put(self):
        ...
        # Optional fields
        teams_name = (
            request_data["teams_name"] if "teams_name" in request_data else None
        )
        ...
        try:
            ...
            status, teams_row = teams_services.update_teams(update_dict)
...
class TeamsByIDOperations(Resource):
    ...
    def delete(self, teams_id):
        ...
        if (
            user_id != UserTypeEnums.SuperAdmin.value
            and str(exist_teams.owner) != user_id
        ):
...
class TeamsUserOperations(Resource):
    ...
    @org_ns2.response(401, "Token is invalid", invlaid_token_response_model)
    @org_ns2.response(500, "Internal Server Error", internal_server_error_model)
    def post(self):
        ...
        if user_services.check_user_teams_mapping(user_details["user_id"], teams_id):
            return {"message": "The user already exists in your teams!"}, 400
